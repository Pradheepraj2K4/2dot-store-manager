...
export default function PurchaseReturnListPage() {
  ...
  const handleDelete = async () => {
    try {
      await purchaseReturnApi.delete(deleteModal.row.id);
  ...
  const filtered = rows.filter((r) => {
    ...
    return (
      ...
      (r.purchase_number ? String(r.purchase_number).includes(q) : false)
    );
  });
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">Returns sent back to suppliers. Stock is deducted automatically.</p>
        </div>
        <button onClick={() => navigate('/purchase-return')} className="btn-primary flex items-center gap-2">
      ...
      <div className="card p-0 overflow-hidden">
        <div className="p-3 border-b border-slate-200 bg-slate-50">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search by return #, bill # or supplier…"
        ...
        {filtered.length === 0 ? (
          <EmptyState icon={ArrowUturnLeftIcon} title="No purchase returns" description="Record your first purchase return to get started" />
        ) : (
            ...
            <table className="w-full text-sm table-zebra">
              <thead>
                <tr className="border-b border-slate-200">
                  ...
                  <th className="px-4 py-2.5 text-left font-semibold text-slate-600">Bill / Purchase #</th>
                  ...
                  <th className="px-4 py-2.5 text-right font-semibold text-slate-600">Value</th>
                ...
                {filtered.map((r) => {
                  ...
                  const rowEls = [
                    <tr key={r.id} className="border-b border-slate-100">
                      ...
                      <td className="px-4 py-2.5 text-slate-600">{billLabel}</td>
                      ...
                      <td className="px-4 py-2.5 text-right font-semibold text-debit-red">{formatCurrency(r.total_amount)}</td>
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-1">
                          <button onClick={() => navigate(`/purchase-return/${r.id}/edit`)} className="text-slate-400 hover:text-trust-blue transition-colors" title="Edit">
