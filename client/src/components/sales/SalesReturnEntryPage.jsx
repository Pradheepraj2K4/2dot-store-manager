...
import { itemApi, salesReturnApi } from '../../api';
...
export default function SalesReturnEntryPage() {
  ...
  useEffect(() => {
    if (!isEdit) return;
    setLoading(true);
    salesReturnApi.getById(returnIdParam)
      .then((res) => {
        const r = res.data;
        ...
        setReason(r.reason || '');
        setNotes(r.notes || '');
        setSaleId(r.sale_id ? String(r.sale_id) : '');
        if (r.ledger_id) setLedger({ id: r.ledger_id, name: r.ledger_name, behaviour: 'customer' });
        setLines(
          (r.items || []).map((l) => ({
            item_id: l.item_id,
  ...
  const handleAddNewItem = (rowIdx) => {
    ...
    const qs = new URLSearchParams({
      returnTo: isEdit ? `/sales-return/${returnIdParam}/edit` : '/sales-return',
    ...
    <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 flex-shrink-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:w-auto w-full">
          ...
          <div>
            <label className="text-xs text-slate-500">Original Sale #</label>
            <input
              type="number"
              value={saleId}
      ...
      <div className="flex-shrink-0 rounded-xl border border-slate-200 bg-white shadow-sm mt-3">
        ...
        <div className="flex items-center justify-end gap-2 px-4 py-3 border-t border-slate-100">
          <button type="button" onClick={() => navigate('/sales-returns')} className="btn-secondary">
            Cancel
          </button>
