...
class ItemController {
  ...
  getBatches(req, res, next) {
    try {
      const batches = itemBatchRepository.getByItem(parseInt(req.params.id));
  ...
  // Next auto-generated batch number for the configured prefix. Used by the
  // purchase entry screen to preview the number that will be assigned when the
  // batch mode is "auto".
  getNextBatchNumber(req, res, next) {
    try {
      const prefix = settingsRepository.get("purchase_batch_prefix") || "";
      const batch_no = itemBatchRepository.getNextAutoNumber(prefix);
      res.json({ success: true, data: { batch_no, prefix } });
    } catch (err) {
      next(err);
    }
  }
  ...
  adjustStocks(req, res, next) {
    try {
      const updated = itemService.adjustStocks(
        req.body && req.body.adjustments,
      );
